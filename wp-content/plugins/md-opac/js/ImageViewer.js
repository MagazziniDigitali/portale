...
ImageViewer.method('initEmbedNavbar', function() {
    ...
    $('#BookReader').append(
        ...
        +   "</span>"
        +   "<span id='BRembedreturn'><p></p></span>"
        + '</div>'
    );
    $('#BRembedreturn p').text(this.bookTitle);
...
url += ':'+location.port;
    ...
    try{
      sequenza = immagine.attributes.getNamedItem(prefixXml+"sequenza").value;
      if (sequenza == null){
        if (prefixXml==""){
          prefixXml="mx-libro:";
        } else {
	...
	if (this.mode==2) {
      ...
      url += '&sequence='+(parseInt(sequenza)+1);
    }
